    ...
    <div class="container py-5">
        <div class="row g-5 align-items-center">  
            ...
            <div class="col-xl-6 col-lg-6 wow fadeInLeft" data-wow-delay="0.2s">
                <div class="h-100">
                    <div class="mb-5">
                        <h1 class="display-4 mb-0">เพิ่มข้อมูลข่าวสาร</h1>
                    </div>
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="npicture" class="form-label">รูปภาพ</label>
                            <input type="file" name="npicture" required class="form-control" id="npicture">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label for="nname" class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="nname" required class="form-control" id="nname">
                        </div>
                        <div class="mb-3">
                            <label for="ndetail" class="form-label">รายละเอียดข่าวสาร</label>
                            <textarea name="ndetail" required class="form-control" id="ndetail"
                                    style="width: 100%; height: 200px; font-size: 16px; padding: 10px; resize: none;">
                            </textarea>
                        </div>
                        <div class="mb-3">
                            <label for="ndate" class="form-label">วันที่ลงข่าวสาร</label>
                            <input type="date" name="ndate" required class="form-control" id="ndate">
                        </div>
                        <button type="submit" name="Submit" class="btn btn-primary btn-md">เพิ่มข้อมูลข่าวสาร</button>
                    </form>
                </div>
            </div>
...
if (isset($_POST['Submit'])) {
    ...
    if (isset($_FILES['npicture']) && $_FILES['npicture']['error'] === UPLOAD_ERR_OK) {
        ...
        $sql = "INSERT INTO `new` (`n_name`, `n_detail`, `n_date`, `n_picture`) 
                VALUES ('$nname', '$ndetail', '$ndate', '')";

        if (mysqli_query($conn, $sql)) {
            $idauto = mysqli_insert_id($conn);
            $new_file_name = "imagenew/" . $idauto . "." . $file_ext;

            // อัปโหลดรูปภาพ
            if (move_uploaded_file($file_tmp, $new_file_name)) {
                // อัปเดตฐานข้อมูลให้มีชื่อไฟล์รูปภาพที่ถูกต้อง
                $update_sql = "UPDATE `new` SET `n_picture`='$new_file_name' WHERE `n_id`='$idauto'";
                mysqli_query($conn, $update_sql);

                echo "<script>alert('เพิ่มข้อมูลข่าวสารสำเร็จ'); window.location='new-admin.php';</script>";
            } else {
