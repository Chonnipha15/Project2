            ...
            <div class="col-xl-6 col-lg-6 wow fadeInLeft" data-wow-delay="0.2s">
                <div class="h-100">
                    ...
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="iimg" class="form-label">รูปภาพ</label>
                            <input type="file" name="iimg" required class="form-control" id="iimg">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label for="iname" class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="iname" required class="form-control" id="iname">
                        </div>
                        <div class="mb-3">
                            <label for="idetail" class="form-label">รายละเอียดข่าวสาร</label>
                            <textarea name="idetail" required class="form-control" id="idetail"
...
if (isset($_POST['Submit'])) {
    ...
    if (isset($_FILES['iimg']) && $_FILES['iimg']['error'] === UPLOAD_ERR_OK) {
        ...
        $sql = "INSERT INTO `it` (`i_name`, `i_detail`, `i_img`) 
                VALUES ('$iname', '$idetail', '')";
        ...
        if (mysqli_query($conn, $sql)) {
            ...
            if (move_uploaded_file($file_tmp, $new_file_name)) {
                ...
                $update_sql = "UPDATE `it` SET `i_img`='$new_file_name' WHERE `i_id`='$idauto'";
                mysqli_query($conn, $update_sql);

                echo "<script>alert('เพิ่มข้อมูลข่าวสารสำเร็จ'); window.location='it-admin.php';</script>";
            } else {
