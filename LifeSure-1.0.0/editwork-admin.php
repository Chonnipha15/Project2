...
$sql1 = "SELECT * FROM `work` WHERE w_id ='{$_GET['wid']}' " ;
                ...
                <div class="h-100">
                    ...
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <input type="file" accept="imagenew/" name="wimg" class="form-control" id="exampleInputimagenew">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="wname" required value="<?=$data1['w_name'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">รายละเอียดข่าวสาร</label>
                            <input type="text" name="wdetail" required value="<?=$data1['w_detail'];?>" class="form-control">
                        </div>
...
if(isset($_POST['Submit'])) {
    ...
    $wdetail = mysqli_real_escape_string($conn, $_POST['wdetail']);
    // ดึงชื่อไฟล์รูปเดิมจากฐานข้อมูล
    $sql_select = "SELECT `w_img` FROM `work` WHERE `w_id` = '$wid'";
    $result = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_assoc($result);
    $old_image = !empty($row['w_img']) ? $row['w_img'] : '';

    // ตรวจสอบว่ามีการอัปโหลดรูปใหม่หรือไม่
    if (isset($_FILES['wimg']) && $_FILES['wimg']['error'] == 0) {
        $file_name = $_FILES['wimg']['name'];
        $file_tmp = $_FILES['wimg']['tmp_name'];
    ...
    if (mysqli_query($conn, $sql_update)) {
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location='work-admin.php';</script>";
    } else {
