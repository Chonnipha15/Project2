...
$sql1 = "SELECT * FROM `it` WHERE i_id ='{$_GET['iid']}' " ;
                ...
                <div class="h-100">
                    ...
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <input type="file" accept="imagenew/" name="iimg" class="form-control" id="exampleInputimagenew">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="iname" required value="<?=$data1['i_name'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">รายละเอียดข่าวสาร</label>
                            <input type="text" name="idetail" required value="<?=$data1['i_detail'];?>" class="form-control">
                        </div>
...
if(isset($_POST['Submit'])) {
    $iid = mysqli_real_escape_string($conn, $_GET['iid']);
    $iname = mysqli_real_escape_string($conn, $_POST['iname']);
    $idetail = mysqli_real_escape_string($conn, $_POST['idetail']);
    // ดึงชื่อไฟล์รูปเดิมจากฐานข้อมูล
    $sql_select = "SELECT `i_img` FROM `it` WHERE `i_id` = '$iid'";
    $result = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_assoc($result);
    $old_image = !empty($row['i_img']) ? $row['i_img'] : '';

    // ตรวจสอบว่ามีการอัปโหลดรูปใหม่หรือไม่
    if (isset($_FILES['iimg']) && $_FILES['iimg']['error'] == 0) {
        $file_name = $_FILES['iimg']['name'];
        $file_tmp = $_FILES['iimg']['tmp_name'];
        ...
        $new_file_name = "imagenew/" . $iid . "." . $file_ext;
    ...
    if (mysqli_query($conn, $sql_update)) {
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location='it-admin.php';</script>";
    } else {
