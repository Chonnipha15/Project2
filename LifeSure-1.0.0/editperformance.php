...
$sql1 = "SELECT * FROM `performance` WHERE P_id ='{$_GET['Pid']}' " ;
    ...
    <div class="container py-5">
        <div class="row g-5 align-items-center d-flex">
            ...
            <div class="col-xl-6 wow fadeInLeft" data-wow-delay="0.2s">
                <div class="h-100">
                    <div class="mb-5">
                        <h1 class="display-4 mb-0">แก้ไขข้อมูลผลงานนิสิต</h1>
                    </div>
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="exampleInputimagenew" class="form-label">รูปภาพ</label>
                            <input type="file" accept="imagenew/" name="Pimg" class="form-control" id="exampleInputimagenew">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="Pname" required value="<?=$data1['P_name'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">รายละเอียดข่าวสาร</label>
                            <input type="text" name="Pdetail" required value="<?=$data1['P_detail'];?>" class="form-control">
                        </div>
...
if(isset($_POST['Submit'])) {
    $nid = mysqli_real_escape_string($conn, $_GET['Pid']);
    $nname = mysqli_real_escape_string($conn, $_POST['Pname']);
    $ndetail = mysqli_real_escape_string($conn, $_POST['Pdetail']);
    // ดึงชื่อไฟล์รูปเดิมจากฐานข้อมูล
    $sql_select = "SELECT `P_img` FROM `performance` WHERE `P_id` = '$Pid'";
    $result = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_assoc($result);
    $old_image = !empty($row['P_img']) ? $row['P_img'] : '';
    ...
    if (isset($_FILES['Pimg']) && $_FILES['Pimg']['error'] == 0) {
        ...
        $new_file_name = "imagenew/" . $Pid . "." . $file_ext;
    ...
    $sql_update = "UPDATE `performance` SET 
                   `P_name` = '$Pname', 
                   `P_detail` = '$Pdetail', 
                   `P_img` = '$image_to_save'
                   WHERE `P_id` = '$Pid'";
    ...
    if (mysqli_query($conn, $sql_update)) {
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location='performance-admin.php';</script>";
    } else {
