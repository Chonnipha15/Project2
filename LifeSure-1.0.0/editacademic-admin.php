...
$sql1 = "SELECT * FROM `academic` WHERE A_id ='{$_GET['Aid']}' " ;
                ...
                <div class="h-100">
                    ...
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            ...
                            <input type="file" accept="imagenew/" name="Aimg" class="form-control" id="exampleInputimagenew">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="Aname" required value="<?=$data1['A_name'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">รายละเอียดข่าวสาร</label>
                            <input type="text" name="Adetail" required value="<?=$data1['A_detail'];?>" class="form-control">
                        </div>
...
if(isset($_POST['Submit'])) {
    ...
    $Adetail = mysqli_real_escape_string($conn, $_POST['Adetail']);
    // ดึงชื่อไฟล์รูปเดิมจากฐานข้อมูล
    $sql_select = "SELECT `A_img` FROM `academic` WHERE `A_id` = '$Aid'";
    $result = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_assoc($result);
    $old_image = !empty($row['A_img']) ? $row['A_img'] : '';

    // ตรวจสอบว่ามีการอัปโหลดรูปใหม่หรือไม่
    if (isset($_FILES['Aimg']) && $_FILES['Aimg']['error'] == 0) {
        $file_name = $_FILES['Aimg']['name'];
        $file_tmp = $_FILES['Aimg']['tmp_name'];
        ...
        $new_file_name = "imagenew/" . $Aid . "." . $file_ext;
    ...
    $sql_update = "UPDATE `academic` SET 
                   `A_name` = '$Aname', 
                   `A_detail` = '$Adetail', 
                   `A_img` = '$image_to_save'
                   WHERE `A_id` = '$Aid'";
    ...
    if (mysqli_query($conn, $sql_update)) {
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location='academic-admin.php';</script>";
    } else {
