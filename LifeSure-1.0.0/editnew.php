...
$sql1 = "SELECT * FROM `new` WHERE n_id ='{$_GET['nid']}' " ;
    ...
    <div class="container py-5">
        <div class="row g-5 align-items-center d-flex">
            ...
            <div class="col-xl-6 wow fadeInLeft" data-wow-delay="0.2s">
                <div class="h-100">
                    <div class="mb-5">
                        <h1 class="display-4 mb-0">แก้ไขข้อมูลข่าวสาร</h1>
                    </div>
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="exampleInputimagenew" class="form-label">รูปภาพ</label>
                            <input type="file" accept="imagenew/" name="npicture" class="form-control" id="exampleInputimagenew">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">หัวข้อข่าวสาร</label>
                            <input type="text" name="nname" required value="<?=$data1['n_name'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">รายละเอียดข่าวสาร</label>
                            <input type="text" name="ndetail" required value="<?=$data1['n_detail'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">วันที่ลงข่าวสาร</label>
                            <input type="date" name="ndate" required value="<?=$data1['n_date'];?>" class="form-control">
                        </div>
...
if(isset($_POST['Submit'])) {
    ...
    $sql_select = "SELECT `n_picture` FROM `new` WHERE `n_id` = '$nid'";
    $result = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_assoc($result);
    $old_image = !empty($row['n_picture']) ? $row['n_picture'] : '';

    // ตรวจสอบว่ามีการอัปโหลดรูปใหม่หรือไม่
    if (isset($_FILES['npicture']) && $_FILES['npicture']['error'] == 0) {
        $file_name = $_FILES['npicture']['name'];
        $file_tmp = $_FILES['npicture']['tmp_name'];
        ...
        // ตั้งชื่อไฟล์ใหม่โดยใช้ `nid`
        $new_file_name = "imagenew/" . $nid . "." . $file_ext;
    ...
    $sql_update = "UPDATE `new` SET 
                   `n_name` = '$nname', 
                   `n_detail` = '$ndetail', 
                   `n_picture` = '$image_to_save'
                   WHERE `n_id` = '$nid'";

    if (mysqli_query($conn, $sql_update)) {
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location='new-admin.php';</script>";
    } else {
