...
$sql1 = "SELECT * FROM `lecturer` WHERE L_id='{$_GET['Lid']}' " ;
    ...
    <div class="container py-5">
        <div class="row g-5 align-items-center d-flex">
            ...
            <div class="col-xl-6 wow fadeInLeft" data-wow-delay="0.2s">
                <div class="h-100">
                    <div class="mb-5">
                        <h1 class="display-4 mb-0">แก้ไขข้อมูลอาจารย์</h1>
                    </div>
                    <form method="post" action="" enctype="multipart/form-data">
                        <div class="mb-3">
                            <label for="exampleInputimage1" class="form-label">รูปภาพ</label>
                            <input type="file" accept="image/*" name="Limg" class="form-control" id="exampleInputimage1">
                            <div class="form-text">กรุณาอัปโหลดรูปภาพ (เฉพาะ JPG, PNG, GIF, ขนาดไม่เกิน 2MB)</div>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">ชื่อ-สกุล</label>
                            <input type="text" name="Lname" required value="<?=$data1['L_name'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">เบอร์โทรติดต่อ</label>
                            <input type="text" name="Ltal" required value="<?=$data1['L_tal'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Email</label>
                            <input type="email" name="Lemail" required value="<?=$data1['L_email'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">ภาระการสอน</label>
                            <input type="text" name="Lteaching" required value="<?=$data1['L_teaching'];?>" class="form-control">
                        </div>
                        <div class="mb-3">
                            <label class="form-label">ประวัติการศึกษา</label>
                            <input type="text" name="Lexper" required value="<?=$data1['L_exper'];?>" class="form-control">
                        </div>
            ...
            <!-- คอลัมน์สำหรับรูปภาพ -->
            <div class="col-xl-6 wow fadeInRight text-center" data-wow-delay="0.4s">
                <img src="img/carousel-2.png" class="img-fluid w-75 rounded shadow" alt="">
            </div>
        </div>
    </div>
</div>
...
if(isset($_POST['Submit'])) {
    ...
    $sql_select = "SELECT `L_img` FROM `lecturer` WHERE `L_id` = '$Lid'";
    $result = mysqli_query($conn, $sql_select);
    $row = mysqli_fetch_assoc($result);
    $old_image = !empty($row['L_img']) ? $row['L_img'] : '';

    // ตรวจสอบว่ามีการอัปโหลดรูปใหม่หรือไม่
    if (isset($_FILES['Limg']) && $_FILES['Limg']['error'] == 0) {
        ...
        // ตั้งชื่อไฟล์ใหม่โดยใช้ `Lid`
        $new_file_name = "image1/" . $Lid . "." . $file_ext;

        // ลบรูปเก่า (ถ้ามีและไม่ใช่ค่าว่าง)
        if (!empty($old_image) && file_exists($old_image)) {
            unlink($old_image);
        }

        // อัปโหลดรูปใหม่ไปยังโฟลเดอร์
    ...
    $sql_update = "UPDATE `lecturer` SET 
                   `L_name` = '$Lname', 
                   `L_tal` = '$Ltal', 
                   `L_email` = '$Lemail',
                   `L_teaching` = '$Lteaching',
                   `L_exper` = '$Lexper',
                   `L_img` = '$image_to_save'
                   WHERE `L_id` = '$Lid'";

    if (mysqli_query($conn, $sql_update)) {
        echo "<script>alert('แก้ไขข้อมูลสำเร็จ'); window.location='lecturer.php';</script>";
    } else {
